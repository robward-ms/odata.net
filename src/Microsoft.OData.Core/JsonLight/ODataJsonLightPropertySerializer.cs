        ...
        internal void WriteTopLevelProperty(ODataProperty property)
        {
            ...
            this.WriteTopLevelPayload(
                () =>
                {
                    ...
                    {
                        ODataContextUrlInfo contextInfo = ODataContextUrlInfo.Create(
            ...
            ODataProperty property)
        {
            ...
            if (this.currentPropertyInfo.IsTopLevel)
            {
                if (this.JsonLightOutputContext.MessageWriterSettings.LibraryCompatibility ==
                    ODataLibraryCompatibility.Version6)
                {
                ...
                else
                {
                    ...
                    // ...
                    // ValidateNullPropertyValue will throw by default but might allow this code to be reached
                    // if validation for this scenario is off. In that case, write "value": null which is not
                    // per spec but the best we could do indexer this case.
                    this.JsonWriter.WriteName(JsonLightConstants.ODataValuePropertyName);
